  ...
  data.days.forEach(function(dayData, dayIndex) {
    ...
    dayData.sections.forEach(function(section) {
      ...
      section.exercises.forEach(function(ex) {
        ...
        var videoBtn = ex.videoId ? '<button class="btn-video" onclick="FitDuo.openVideo(\'' + ex.videoId + '\')" title="Watch form video">' + PLAY_SVG + '</button>' : '<span></span>';
...
window.FitDuo = {
  ...
  closeVideo: function(event) {
    if (event && event.target !== event.currentTarget && !event.target.classList.contains('video-close')) return;
